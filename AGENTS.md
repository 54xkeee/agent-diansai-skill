...
普通电赛/校赛项目、初学者求助和现场调试优先使用 `skills/diansai-collab/SKILL.md`。AI主动读工程并作技术判断，人提供目标和现场事实；重点维护物理映射、目标语义、实际版本与已纠正的区别。重复失败时改变取证方式，迁移时保留目标和工况边界，不只追加参数补丁。不要因为出现赛题PDF就自动长跑或创建整套文档。
